...
def next_minima(a, current, n=1):
    """
    Find the next local minima in a (must be less than n adjacent values in each direction)
    """
...
def next_maxima(a, current, n=1):
    """
    Find the next local maxima in a (must be less than n adjacent values in each direction)
    """
    for i in range(current+1, len(a)-1):
        forward_neighbours = min(n, len(a) - (i+1))
        backward_neighbours = min(n, i)
...
def moving_average(s, n=5000):
    """
    For each point in numpy array s, return the moving average of the previous n points.
    """
    avg = np.zeros(len(s))
